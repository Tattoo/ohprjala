...
public class Laskin extends javax.swing.JFrame implements ActionListener, KeyListener {
	 ...
	 * @param args Doesn't take arguments
	 *************************************************/
	public static void main(String[] args) {
		java.awt.EventQueue.invokeLater(
				new Runnable() { public void run() { new Laskin().setVisible(true); } }
		);
	}


	/** ************************************************
	 * Sole constructor. Creates a calculator.
	 ***************************************************/
	public Laskin() {   
		super("Calculator");
	...
	{ 
		...
		// Cause UI button corresponding the key typed to be activated, if applicable
		switch(key) {
		case '0':  { button_0.doClick(BUTTONPRESSEDTIME); break; }
		case '1':  { button_1.doClick(BUTTONPRESSEDTIME); break; }
		case '2':  { button_2.doClick(BUTTONPRESSEDTIME); break; }
		case '3':  { button_3.doClick(BUTTONPRESSEDTIME); break; }
		case '4':  { button_4.doClick(BUTTONPRESSEDTIME); break; }
		case '5':  { button_5.doClick(BUTTONPRESSEDTIME); break; }
		case '6':  { button_6.doClick(BUTTONPRESSEDTIME); break; }
		case '7':  { button_7.doClick(BUTTONPRESSEDTIME); break; }
		case '8':  { button_8.doClick(BUTTONPRESSEDTIME); break; }
		case '9':  { button_9.doClick(BUTTONPRESSEDTIME); break; }
		case '+':  { button_add.doClick(BUTTONPRESSEDTIME); break; }
		case '-':  { button_substraction.doClick(BUTTONPRESSEDTIME); break; }
		case '*':  { button_multiply.doClick(BUTTONPRESSEDTIME); break; }
		case '/':  { button_divide.doClick(BUTTONPRESSEDTIME); break; }
		case '^':  { button_power.doClick(BUTTONPRESSEDTIME); break; }
		case ',':  { button_decimalPoint.doClick(BUTTONPRESSEDTIME); break; }
		case '.':  { button_decimalPoint.doClick(BUTTONPRESSEDTIME); break; }
		case '=':  { button_equals.doClick(BUTTONPRESSEDTIME); break; }
		case '\r': { button_equals.doClick(BUTTONPRESSEDTIME); break; }
		case '\n': { button_equals.doClick(BUTTONPRESSEDTIME); break; }
		case '\b': { button_backspace.doClick(BUTTONPRESSEDTIME); break; }
		}
	} 
	...
	/**
	 * Equation
	 * Holds the values for visual text output and does the actual math.
	 */
	private class Equation { 
		 ...
		 */
		public void clear() {
			pastValues.add( new Double(currentValue) ); // store currentValue to pastValues
			currentValue = 0;
			currentDigits = "";
			currentDigitsIsEven = true;
			currentDigitsIsPositive = true;
		...
		public void undo() {
			...
			try { currentValue = pastValues.remove( pastValues.size()-1 ).doubleValue(); }
			catch (Exception e) { System.out.println("Programming error: " + e); } // remove() throws if nothing to remove
		...
		public void calculate() { 
			...
			case '+':
			{
				pastValues.add( new Double(currentValue) );  // store the currentValue to pastValues
