...
public class MemoryTextGenerator {
	...
	/**
	 * @param slots
	 *            the order of slots, for example 1, 2, 3, 4, 5
	 * @param maxLenght
	 *            the maximium length of value shown, for example if maxlenght
	 *            is 5, then 1,555555556 is show as 1,555
	 * @param maxRows
	 *            the maximum number of rows. If the number of slots exeeds
	 *            maxrows, the rest of the slots are printed to a second column
	 */
	public MemoryTextGenerator(int[] slots, int maxLenght, int maxRows) {
		this.slots = slots;
		this.maxLength = maxLenght;
		if (maxRows > slots.length)
			this.maxRows = slots.length;
		else
